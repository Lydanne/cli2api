...
- `@cli2api/agents-sdk`: provider-neutral contracts, process runtime, and the `AgentsSDK` facade.
...
2. Core resolves the requested model profile.
3. Quota checks run before execution.
4. The selected provider emits normalized events.
...
New provider packages should implement `AgentProvider`, normalize provider events into `AgentEvent`, and keep provider-specific configuration inside profile metadata.
