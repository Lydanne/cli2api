...
import { createUpstreamHarness } from "./testing/upstream-harness.js";

class RecordingAgentProvider implements AgentProvider {
  public readonly type = "mock";

  public readonly inputs: AgentRunInput[] = [];
  ...
  public async *run(input: AgentRunInput): AsyncIterable<AgentEvent> {
    ...
    if (input.conversation) {
      yield {
        ...
        providerSessionId: `provider:${input.profile.cwd}`,
        metadata: { cwd: input.profile.cwd }
      };
    }
    yield { type: "run.completed", runId: input.runId, output: "ok" };
  }
}
...
describe("@cli2api/core upstream account pool", () => {
  ...
  it("persists provider session ids only for explicit downstream session identity", async () => {
    await createAuthenticatedAccount("acct-provider-session");
    await createInstance("inst-provider-session", "acct-provider-session", { maxConcurrentRuns: 2 });
    const profile = await createProfile("mock-provider-session");
    const key = await createApiKey("provider-session-key");

    await createRun(key.token, {
      prompt: "explicit provider session",
      profileId: profile.id,
      user: "learner-provider",
      metadata: { sessionId: "chat-provider" }
    });
    await createRun(key.token, {
      prompt: "default stateless provider session",
      profileId: profile.id
    });
    ...
    expect(sessions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          userId: "learner-provider",
          sessionId: "chat-provider",
          providerSessionId: expect.stringContaining("learner-provider"),
          providerSessionMetadata: expect.objectContaining({ provider: "mock" })
        }),
        expect.objectContaining({
          userId: "default",
          sessionId: "default",
  ...
  it("rebinds a session when the pinned upstream instance becomes unhealthy", async () => {
    ...
    expect(recordingProvider.inputs).toHaveLength(2);
    expect(recordingProvider.inputs[0]?.conversation?.providerSessionId).toBeUndefined();
    expect(recordingProvider.inputs[1]?.conversation?.providerSessionId).toBeUndefined();
    ...
    expect(await sessionsResponse.json()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          providerSessionId: `provider:${recordingProvider.inputs[1]?.profile.cwd}`,
          runCount: 2
        })
      ])
    );
  ...
  it("temporarily overflows a full pinned session without moving the pin", async () => {
    ...
    expect(recordingProvider.inputs).toHaveLength(2);
    expect(recordingProvider.inputs[0]?.conversation?.providerSessionId).toBeUndefined();
    expect(recordingProvider.inputs[1]?.conversation).toBeUndefined();
    const sessionsResponse = await harness.app.handle(
      new Request("http://localhost/api/admin/upstream/run-sessions", {
        headers: { cookie: harness.cookie }
      })
    ...
    expect(await sessionsResponse.json()).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          upstreamInstanceId: firstRun.upstreamInstanceId,
          providerSessionId: `provider:${recordingProvider.inputs[0]?.profile.cwd}`,
          runCount: 2
        })
      ])
    );
